...
# Run the drone demo: relay + drone, then use `just control <id>` to connect
drone-demo:
    #!/usr/bin/env bash
    set -e
    trap 'kill $(jobs -p) 2>/dev/null; wait' INT TERM
    ...
    echo "  Run 'just control $DRONE_ID' in another terminal to send commands."
...
# Connect the controller to a drone by ID
control drone_id:
    DRONE_ID={{ drone_id }} cargo run --bin controller
